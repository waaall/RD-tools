...
class SettingWindow(QWidget):
    ...
    def _initialize_all_pages(self):
        for page_name in self.main_categories:
            ...
            page_widget = QWidget()
            page_display.setMinimumSize(self.window_minimum_size[1],
                                        self.window_minimum_size[2])
    ...
    def __initialize_page(self, setting_map, page_layout):
        ...
        for name, options_and_path in setting_map.items():
            ...
            if len(path) == 2:  # 直接属于 main_category 的设置
                ...
                group_name = path[1]
                if not any(isinstance(w, QGroupBox) and w.title() == group_name
                           for w in page_layout.parentWidget().findChildren(QGroupBox)):
                    ...
                    group_box = next(w for w in page_layout.parentWidget().findChildren(QGroupBox)
                                     if w.title() == group_name)
    ...
    def __add_setting(self, name, value, control_type="text", options=None, layout=None):
        if control_type == "combo" and options:
            ...
            combo.currentIndexChanged.connect(lambda index:
                                              self._combo_changed_handler(combo,
                                                                          name,
                                                                          index,
                                                                          options[0]))
            # 设置Label和combo的样式
            ...
            text_setting.textChanged.connect(lambda:
                                             self.update_setting(name, text_setting.text()))
            # 设置Label和checkbox的样式
            label = QLabel(name)
